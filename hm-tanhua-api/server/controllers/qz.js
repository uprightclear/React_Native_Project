const crypto = require('crypto'),
    ...
    { getBaiDuAiAccessToken, checkImage } = require('../utils/kits.js'),
    fs = require('fs'),
    path = require('path'),
    https = require('https'),
...
class qzController {
    ...
    static async recommend(ctx) {
        ...
        select u.*,t.tid,t.content,t.star_count,t.comment_count,t.like_count,t.create_time from (
            SELECT t.id as uid, t.mobile,t.header ,t.nick_name,t.gender,t.age,t.guid,
            t.xueli,t.marry,t.dist 
            FROM (
                SELECT *,ABS(Distance - ${currentUserDistance}) AS dist FROM dt_users u
                WHERE u.status = 0 AND NOT EXISTS(select 1 from dt_like k where k.uid = u.id AND k.type='dislike')
                ) AS t
    ...
    static async newtrends(ctx) {
        try {
            ...
            // 2.0 获取符合条件的数据总条数
            let queryCount = `
            select count(1) as count from dt_trends t 
            inner join dt_users u on (u.id=t.uid and t.status = 0
            AND NOT EXISTS(select 1 from dt_like k where k.uid = u.id AND k.type='dislike'))
            order by t.create_time DESC
        `;
            ...
            let sql = `
             select u.*,t.tid,t.content,t.star_count,t.comment_count,t.like_count,t.create_time from (
                 SELECT t.id as uid,t.mobile, t.header ,t.nick_name,t.gender,t.age,
                 t.xueli,t.marry,t.dist 
                 FROM (
                     SELECT *,ABS(Distance - ${currentUserDistance}) AS dist FROM dt_users u
                     WHERE u.status = 0 AND NOT EXISTS(select 1 from dt_like k where k.uid = u.id AND k.type='dislike')
            ...
            let tids = trends.map(item => item.tid).join(',');
            // console.log(tids);
            let albums = await ctx.executeSql(`select tid,  thum_img_path, org_img_path 
        from dt_album where tid in (${tids})`);
    ...
    static async star(ctx) {
        try {
            ...
            let sql = `
                       update dt_trends set star_count =star_count + 1 where tid = ${params.id};                      
                    `;
            await ctx.executeSql(sql);

            await ctx.executeSql(`INSERT INTO dt_star
            (
              uid
             ,star_uid
             ,tid
    ...
    static async like(ctx) {
        try {
            ...
            let sql = `
                       update dt_trends set like_count =like_count + 1 where tid = ${params.id};                      
                    `;
            await ctx.executeSql(sql);

            await ctx.executeSql(`INSERT INTO dt_like
            (
              uid
             ,like_uid
             ,tid
    ...
    static async noInterest(ctx) {
        try {
            ...
            let res1 = await ctx.executeSql(`select uid from dt_like where tid = ${params.id} and
            like_uid=${currentUserID} and uid=${res[0].uid} and type='dislike'`);
    ...
    static async comments(ctx) {
        try {
            ...
            SELECT dc.*,du.header, du.nick_name FROM dt_comments dc 
            inner join dt_users du on(dc.uid = du.id)
            WHERE dc.tid = ${params.id} and dc.parent_cid = 0 ORDER BY dc.create_time DESC
            LIMIT ${startIndex},${pagesize}
            `)
    ...
    // 提交评论
    static async commentssubmit(ctx) {
        try {
    ...
    // 上传动态中的图片
    static async trendsimageupload(ctx) {
        try {
            ...
            if (!files.path) {
                ...
                files.map(file => {
                    ...
                    datares.push({
                        ...
                        headImgPath: global.domain + `/upload/album/${mobile}/` + `${filename + extname}`
                    });
                })
